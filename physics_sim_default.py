...
class PhysicsSim():
    ...
    def reset(self, positionNoise=0, velocityNoise=0, angleNoise=0, angleRateNoise=0):
        ...
        angularNoise = (np.random.rand(3)-0.5)*(angleNoise/180*np.pi)
        angleSigns = np.sign(angularNoise)
        self.pose = np.array([0.0, 0.0, 10.0, 0.0, 0.0, 0.0]) if self.init_pose is None else np.copy(self.init_pose + np.concatenate((((np.random.rand(3)-0.5)*positionNoise),(angularNoise))))
        # Make sure the angles are within [0, 2*pi].
        self.pose[3:6] = [a if a>=0 else 2*np.pi-a for a in self.pose[3:6]]
        ...
        self.angular_v = np.array([0.0, 0.0, 0.0]) if self.init_angle_velocities is None else np.copy(self.init_angle_velocities + ((np.random.rand(3)-0.5)*(angleRateNoise/180*np.pi))*angleSigns)
