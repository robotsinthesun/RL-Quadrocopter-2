...
class PhysicsSim():
    def __init__(self, init_pose=None, init_velocities=None, init_angle_velocities=None, runtime=5.):
        ...
        # Added according to https://discussions.udacity.com/t/quadcopter-third-euler-angle-is-not-changing/654397/2
    ...
    def reset(self, positionNoise=0, velocityNoise=0, angleNoise=0, angleRateNoise=0):
        self.time = 0.0
        # Make sure the angle noise corresponds to the angular velocity noise.
        posNoise = (np.random.rand(3)-0.5)*2*positionNoise
        posNoise[1] = 0.
        velNoise = (np.random.rand(3)-0.5)*2*velocityNoise
        velNoise[1] = 0.
        angNoise = (np.random.rand(3)-0.5)*2*(angleNoise/180*np.pi)
        angNoise[[0,2]] = 0.
        angSigns = np.sign(angNoise)
        angRateNoise = (np.random.rand(3)-0.5)*2*(angleRateNoise/180*np.pi)#*angSigns
        angRateNoise[[0,2]] = 0.
        self.pose = np.array([0.0, 0.0, 10.0, 0.0, 0.0, 0.0]) if self.init_pose is None else np.copy(self.init_pose + np.concatenate((posNoise, angNoise)))
        # Make sure the angles are within [0, 2*pi].
        self.pose[3:6] = self.pose[3:6] % (np.pi*2)
        self.v = np.array([0.0, 0.0, 0.0]) if self.init_velocities is None else np.copy(self.init_velocities + velNoise)
        self.angular_v = np.array([0.0, 0.0, 0.0]) if self.init_angle_velocities is None else np.copy(self.init_angle_velocities + angRateNoise)
    ...
    # Change moments calculation according to https://discussions.udacity.com/t/quadcopter-third-euler-angle-is-not-changing/654397/2
    '''
